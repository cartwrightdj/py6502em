...
class AddressSpace:
    def __init__(self,start:int,end:int,maxvalue=0xFF,ro=False,default_value=0x00,name='AddressSpace'):
        ...
        self.ro = ro
        self.name = name
        self.data = {address: default_value for address in range(start,end+1)}
        self.uuid = str(uuid.uuid4()).replace("-", "")[:8]

        self.f = open(f'{name}_.mem', 'wb')
    ...
    def _load_from_bf(self,filename,start) -> dict:
       ...
       # assert False
        if os.path.exists(filename[:-6]+'.sym'):
            symbols = {}
            print(f"Loading symbols from file")
            with open(filename[:-6]+'.sym', 'r') as f:
                lines = f.readlines()
                for line in lines:
                    key, value = line.split()
                    #fprint(f"{value}:{key}")
                    symbols[f"{int(key.strip(),16)}"] = value.strip()
...
class CPU:
    ...
    def step(self):
        ...
        opcodesym, addrsym = '', ''  # If symbols present we will load these
        opcode = self.read(self.pc)
        if str(self.pc) in self.symbols.keys(): 
            opcodesym = Ansi.FG_BRIGHT_GREEN + "(" + self.symbols[f"{self.pc}"].strip() + ")" 
        else:
        ...
        if opcode not in self.opcode_table:
            ...
            try:
                ...
                if addr is not None:
                    ...
                    pass
                if str(addr) in self.symbols.keys():
                    addr_str += f"  ({self.symbols[str(addr)]})"
                sp = self.sp
                stack = []
                stack_str = ''
...
cpu = CPU() 
mmu = MMU()
rom = AddressSpace(0x8000,0xFFFF,name='ROM')
sharedmem = AddressSpace(0x7000,0x8000-1,name='Sharedmemory')
cpu.symbols = rom._load_from_bf('rom.bin',0x8000)
#cpu.symbols = rom._load_from_bf('rom2.bin',0xA000)
rom._dump()

#rom._dump()
cpu.memory.add(rom)
cpu.memory.add(sharedmem)
cpu.memory.watch(0x402)

memmap = MappedMemory(0x00,0x7000-1)
v_mem0 = AddressSpace(0x00,0x7000-1,name='SystemMem')
v_mem1 = AddressSpace(0x00,0x7000-1,name='v_mem1')
v_mem2 = AddressSpace(0x00,0x7000-1,name='v_mem2',default_value=0xFF)

memmap.map(v_mem0,0x8000)
memmap.map(v_mem1,0x8001)
memmap.map(v_mem2,0x8002)
cpu.memory.add_virtual(memmap)
